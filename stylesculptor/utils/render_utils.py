...
def render_frames(sample, extrinsics, intrinsics, options={}, colors_overwrite=None, verbose=True, **kwargs):
    if isinstance(sample, Octree):
        ...
        renderer.rendering_options.far = options.get('far', 80)
        renderer.rendering_options.ssaa = options.get('ssaa', 4)
    ...
    for j, (extr, intr) in tqdm(enumerate(zip(extrinsics, intrinsics)), desc='Rendering', disable=not verbose):
        if not isinstance(sample, MeshExtractResult):
            ...
            renderer.rendering_options.bg_color = options.get('bg_color', (1, 1, 1))
            res = renderer.render(sample, extr, intr, return_types = ["mask", "normal", "depth", "color"])
...
def render_video(sample, resolution=1024, bg_color=(1, 1, 1), num_frames=300, r=2, fov=40, **kwargs):
